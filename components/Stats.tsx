...
export default function Stats() {
  ...
  useEffect(() => {
    ...
    const centerPlugin = {
      ...
      beforeDraw: function(chart: any) {
        ...
        ctx.fillText("QUESTIONS SOLVED", width / 2, height / 2 + 20);
        ctx.save();
      }
    };
  ...
  useEffect(() => {
    ...
    if (apiData) {
        if(platform === 'leetcode') { chartData = generateNoise(apiData.leetcode?.rating || 1500); color = THEME.leetcode; }
        if(platform === 'codeforces') { chartData = generateNoise(apiData.codeforces?.rating || 800); color = THEME.codeforces; }
        if(platform === 'codechef') { chartData = generateNoise(apiData.codechef?.rating || 1000); color = THEME.primary; }
        if(platform === 'geeksforgeeks') { chartData = generateNoise(apiData.geeksforgeeks?.problems_solved_total || 500); color = THEME.gfg; }
    }
    ...
    <section id="about" style={{position: 'relative', zIndex: 2}}>
      ...
      <div className="bento-grid">
        ...
        <div className="sci-card" style={{minHeight: '350px'}}>
            ...
            {/* Custom Legend */}
            <div style={{display: 'flex', justifyContent: 'center', gap: '15px', marginTop: '10px', fontSize: '0.75rem', color: THEME.textDim}}>
        ...
        <div className="sci-card">
            ...
            {/* CHART VIEWPORT */}
            <div style={{ width: '100%', height: '180px', marginBottom: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      {/* Decorative arrow */}
      <img src="/assets/arrow.svg" alt="Next" className="icon arrow" onClick={() => location.href = './#company'} />
    </section>
  );
}