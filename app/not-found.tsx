...
export default function NotFound() {
      ...
      case "help":
        outputNode = (
          <div className="line">
            ...
            <span className="yellow">help</span> - Show menu | <span className="yellow">whoami</span> - Info | <span className="yellow">report</span> - Log error | <span className="yellow">exit</span> - Home
          </div>
        );
        break;
      case "whoami":
        outputNode = <div className="line">User: <span className="blue">Guest</span> | Session: <span className="green">Active</span></div>;
        break;
      case "report":
  ...
  // --- WINDOW CONTROLS ---
  const handleClose = (e: React.MouseEvent) => { e.stopPropagation(); router.push("/"); };
  const handleMinimize = (e: React.MouseEvent) => { e.stopPropagation(); setWindowState("min"); };
    ...
    <div className={`not-found-wrapper ${windowState}`} onClick={() => inputRef.current?.focus()}>
      ...
      >
        ...
        <div className="Terminal_body" ref={bodyRef}>
          <div className="line">Last login: {new Date().toLocaleDateString()} on <span className="purple">ttys001</span></div>
          <div className="line" style={{ marginTop: '10px' }}>
            Connection: <span className="green">{data ? data.ip : "Loading..."}</span> via <span className="blue">{data ? data.isp : "..."}</span>.
            <br />
            Location: <span className="yellow">{data ? data.city : "..."}</span> ({data ? data.timezone : "..."}).
          </div>
          <div className="line" style={{ marginTop: '10px', marginBottom: '20px' }}>
            <span className="red">[ERROR 404]</span> Target not found: <span className="dim">{currentUrl}</span>
            <br/>System halted. Type <span className="yellow">help</span> for options.
          </div>

          {history.map((item, i) => (
            <div key={i}>
              <div className="line"><span className="green">➜</span> <span className="blue">~</span> <span className="dim">{item.cmd}</span></div>
              {item.output}
            </div>
          ))}
